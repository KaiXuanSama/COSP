package com.kaixuan.copilot_ollama_proxy.provider.agnes.ollama;
...
/**
 * Agnes Ollama 协议实现 —— 将 Ollama 格式请求转换为 OpenAI 格式调用 Agnes API，
 * 再将 OpenAI 格式响应转换回 Ollama 格式。
 * 所有运行时配置（API Key、Base URL、模型列表）均从数据库读取。
 */
@Service
public class AgnesOllamaService extends AbstractRuntimeCatalogOllamaService {

    private static final Logger log = LoggerFactory.getLogger(AgnesOllamaService.class);
    ...
    public AgnesOllamaService(RuntimeProviderCatalog runtimeProviderCatalog, @Value("${agnes.default-model:agnes-2.0-flash}") String fallbackDefaultModel, ObjectMapper objectMapper,
            WebClient.Builder webClientBuilder) {
        super(runtimeProviderCatalog, fallbackDefaultModel);
        this.transportClient = new OpenAiTransportClient(runtimeProviderCatalog, webClientBuilder, new OpenAiTransportClient.Config("agnes", "https://apihub.agnes-ai.com/v1", "/chat/completions",
    ...
    private List<String> buildCapabilitiesFromDb(String resolvedModel) {
        ...
        try {
            ...
            log.warn("Agnes 模型 [{}] 能力读取失败，仅使用默认 completion 能力: {}", resolvedModel, e.getMessage());
        }
        return caps;
    }
    ...
    public Mono<OllamaChatResponse> chat(OllamaChatRequest request) {
        ...
        log.info("Agnes Ollama→OpenAI，模型: {}, 流式: false", openAiRequest.get("model"));
    ...
    private OllamaChatResponse convertOpenAiToOllama(String openAiJson, String requestModel) {
        try {
            ...
            log.error("Agnes OpenAI → Ollama 转换失败", e);
