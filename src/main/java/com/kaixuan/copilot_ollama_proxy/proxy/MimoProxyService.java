...
import com.kaixuan.copilot_ollama_proxy.anthropic.*;
 ...
 */
@Service
public class MimoProxyService {

    private static final Logger log = LoggerFactory.getLogger(MimoProxyService.class);

    /** 用于调用 Mimo 后端 Anthropic 兼容接口的 HTTP 客户端 */
     ...
     */
    public MimoProxyService(@Value("${mimo.api-key}") String apiKey, @Value("${mimo.base-url}") String baseUrl,
    ...
    public Mono<OllamaChatResponse> chat(OllamaChatRequest request) {
        ...
        return webClient.post().uri("/v1/messages").bodyValue(anthropicReq).retrieve()
                .bodyToMono(AnthropicResponse.class).map(resp -> convertResponse(resp, request.getModel()));
    }
    ...
    public Flux<OllamaChatResponse> chatStream(OllamaChatRequest request) {
        ...
        return webClient.post().uri("/v1/messages").contentType(MediaType.APPLICATION_JSON).bodyValue(anthropicReq)
