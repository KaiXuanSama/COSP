...
CREATE TABLE IF NOT EXISTS provider_model (
    ...
    -- 最大输出配置（V9 JSON）：{"max_output_tokens":64000,"overwrite_mode":"override|fallback"}
    -- 只有两档模式：override 一律用配置值；fallback 用下游的、没带才补。
    -- 没有 passthrough / delete：Anthropic 侧 max_tokens 缺失会 400，「不补」在那条线路上等于必然失败。
    max_output_tokens TEXT       NOT NULL DEFAULT '{"max_output_tokens":64000,"overwrite_mode":"fallback"}'
