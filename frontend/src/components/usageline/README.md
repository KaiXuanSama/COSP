...
| range | 标题 | 副标题 |
|---|---|---|
| `7d` | 近 **N** 日 token 用量 | 按天汇总 · 纵轴为 token 数 |
| `1d` | 今日时段 token 用量 | 05:00 起算 24 小时 · 每点含前后半小时 · 纵轴为 token 数 |

**`7d` 标题里的天数取自 `points.length`，不是写死的 7。** 区间形态的跨度可由日期选择器改变（拖手柄能拖出 6 天或 9 天），写死会让标题与图上的柱数不符 —— 而这种不符没有任何报错，只是标题在撒谎。`points` 为空时退回「近期」，因为那时「近 0 日」是明显的错话。
