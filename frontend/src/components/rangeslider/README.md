...
开启后单击轨道上任一位置即跳过去 —— 落点算式因形态而异：

**区间形态**移<strong>更近的那个手柄</strong>。
...
**单点形态**没有手柄也没有跨度，点哪儿整点就挪哪儿 —— 指哪儿打哪儿，无需距离计算（落点即目标）。它走 `slideTo`（宽度为 1 的块平移）而非 `jumpTo`（在单点下会被两条约束夹死、返回原值）。
...
| 有效手势 | 拖手柄改跨度、拖块平移、点击跳转 | 拖动、点击跳转（整点挪过去） |
| 键盘焦点 | 块 + 两个手柄 | 只有块 |
...
**形态切换有交叉淡化。** 两种形态之间切换时四件事同时发生、共用 `$morph-duration`：

| | 区间 → 单点 |
|---|---|
| 块宽度 | 345px → 28px（走 `$slide-duration`） |
| 胶囊底色 | opacity 1 → 0 |
| 端点手柄 | opacity 1 → 0，同时 `scale(1)` → `scale(0.6)` |
| 圆点 | opacity 0 → 1，同时 `scale(0.6)` → `scale(1)` |

实测中途某帧：宽 140px、胶囊 0.29、圆点 0.71、手柄 0.29 —— 四者严格同步。手柄缩小淡出与圆点放大淡入方向相反，交叉那一刻不会有半透明重叠的浑浊感。

这个动画是为「折线图切换今日时段 / 近 N 日时联动」准备的，那是高频可见的操作。
...
| 单击轨道（需 `clickToJump`） | 区间：更近的手柄移到该位置；单点：整点挪过去。见「点击跳转」 |
...
| `$morph-duration` | `0.16s` | 单点 ↔ 区间形态切换的淡化时长。与上者同值但语义不同（形态柔和度 vs 位移跟手度），可独立调 |
...
**单点形态的圆是独立元素，不是伪元素。** 块的 `::before` 已被胶囊底色占用（那层要能淡出）。更根本的是：`content` 从 `none` 变 `""` 时元素凭空出现，没有起始帧可供过渡 —— 早先正是这么写的，切换时圆是硬闪出来的。

**胶囊底色画在 `::before` 上而非块自身。** `background` 是复合简写，渐变与 `none` 之间无法插值，直接覆写只会在第 0 帧硬切。挪到伪元素后就能用 `opacity` 过渡。这与柱状图那条「底色用 `background-color` 而非 `background`」是同一类问题的两种解法 —— 那里换的是可插值的属性，这里换的是承载它的元素。

**单点形态下手柄留在 DOM 里，只是淡出。** 用 `v-if` 摘掉没有退场帧，切换时手柄会凭空消失而块还在慢慢收缩，观感是「先没了再变形」。淡出时必须同时关掉 `pointer-events` —— 完全透明的元素照样拦截点击，不关的话单点形态下用户点到的是两个看不见的手柄而非可拖的块。`disabled` 与 `tabindex` 也要跟着关，键盘 Tab 不该停在一个不起作用的按钮上。

**手柄的 `--hidden` 规则必须写在 `hover` / `dragging` 之后。** 切换途中鼠标可能正停在块上，那两个状态仍挂着；若被它们覆盖，手柄会一边淡出一边放大，与圆点的入场撞在一起。同理，覆写 `transform` 时要重申各自的居中位移（`-50%` / `+50%` 两侧不同）。
