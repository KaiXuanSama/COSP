...
/**
 * 柱列的横向内边距（px）—— hover 高亮块比柱体略宽出的部分。
 *
 * 必须由 JS 按权重缩放后写成内联样式，不能固定写在 CSS 里：
 * {@code flex-basis: 0} 只压缩内容盒，padding 不参与 flex 收缩，
 * 于是退场柱在权重趋近 0 时仍有 {@code 2 × COLUMN_PAD_X} 的宽度地板，
 * 被移除的瞬间这部分空间一次性释放，留存柱就会「弹」一下。
 */
export const COLUMN_PAD_X = 2

/**
 * 柱列之间的间距（px）。
 *
 * 由每根柱子以左右各半的外边距表达，而非容器的 {@code gap} ——
 * gap 由容器分配，退场柱只要还在 DOM 里就占着一整个 gap 位，
 * 无法随权重收缩，同样会在移除时造成跳变。
 */
